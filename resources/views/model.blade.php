                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <div class="d-flex gap-2 align-items-center">
                        ...
                        @if($items->count() > 0)
                            ...
                            <form action="{{ route('trashcan.bulk-force-delete', $encoded) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="ids" id="deleteIds">
                                <button type="submit" class="btn btn-danger btn-sm bulk-btn" disabled
                                        onclick="return confirm('Permanently delete selected items?') && (document.getElementById('deleteIds').value = JSON.stringify(getSelectedIds()))">
                                    <i class="bi bi-trash3 me-1"></i>Delete
                                </button>
                            </form>
                            <form action="{{ route('trashcan.empty-trash', $encoded) }}" method="POST" class="d-inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-outline-danger btn-sm"
                                        onclick="return confirm('Empty all trash for {{ $activeModel['name'] }}?')">
                                ...
                                @foreach($items as $item)
                                    <tr>
                                        ...
                                        <td class="text-end">
                                            ...
                                            <form action="{{ route('trashcan.force-delete', [$encoded, $item->id]) }}" method="POST" class="d-inline">
                                                @csrf
                                                @method('DELETE')
                                                <button type="submit" class="btn btn-sm btn-outline-danger" title="Delete permanently" onclick="return confirm('Permanently delete?')">
                ...
                <div class="flex justify-between items-center mb-6">
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        @if($items->count() > 0)
                            ...
                            <form action="{{ route('trashcan.bulk-force-delete', $encoded) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <input type="hidden" name="ids" id="deleteIds">
                                <button type="submit" class="bulk-btn px-4 py-2 bg-red-500 text-white text-sm rounded-lg hover:bg-red-600 disabled:opacity-50 disabled:cursor-not-allowed"
                                        disabled onclick="return confirm('Permanently delete?') && (document.getElementById('deleteIds').value = JSON.stringify(getSelectedIds()))">
                                    <i class="ri-delete-bin-line mr-1"></i>Delete
                                </button>
                            </form>
                            <form action="{{ route('trashcan.empty-trash', $encoded) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="px-4 py-2 border border-red-300 text-red-600 text-sm rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20"
                                        onclick="return confirm('Empty all trash?')">
                            ...
                            @foreach($items as $item)
                                <tr class="hover:bg-gray-50 dark:hover:bg-slate-700/50 transition-colors">
                                    ...
                                    <td class="p-4 text-right">
                                        ...
                                        <form action="{{ route('trashcan.force-delete', [$encoded, $item->id]) }}" method="POST" class="inline">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" class="p-2 text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" onclick="return confirm('Permanently delete?')"><i class="ri-delete-bin-line"></i></button>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
